...
RP_BROADCAST_HEADER = BROADCAST_HEADER;

TRP2_BROADCAST_COMMANDS = {
	HELLO = "TRP2HELLO",
	GetLocalCoord = "CSCAN",
...
function TRP2_ReceiveMessageChannel(message, sender)
	...
	local messageTab = TRP2_fetchInformations(message, BROADCAST_SEPARATOR);
	...
	if messageTab[2] == TRP2_BROADCAST_COMMANDS.GetLocalCoord then
		if TRP3_API and TRP3_API.globals.version > 10 then
			return;
		end
...
function TRP2_SendP2PMessage(target, command, ...)
	local message = command;
	for _, arg in pairs({ ... }) do
		arg = tostring(arg);
		if (arg:find(BROADCAST_SEPARATOR)) then
			TRP2_debug("TRP2_SendP2PMessage ERROR : Tried to send separator character ~ ");
			return;
		end
	...
	if message:len() < 254 then
		...
		TRP2_debug("TRP2_SendP2PMessage ERROR : Message is to big for protocol")
	end
end
...
function TRP2_OnP2PMessageRecevied(message, sender)
	local messageTab = TRP2_fetchInformations(message, BROADCAST_SEPARATOR);

	if messageTab[1] == TRP2_BROADCAST_COMMANDS.GetLocalCoord and playersCanSeeEachOthers(sender) then
		if tonumber(messageTab[4]) ~= TRP2_LatestMapCoordZoneID then
			return;
		end
		TRP2_AddPlayerToMapTab(sender, { tonumber(messageTab[2]), tonumber(messageTab[3]), messageTab[5] });
	end
end
