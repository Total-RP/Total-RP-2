...
function TRP2_InitialisationUI()
	...
	TRP2_GuideFrameTexteA:SetFont("h1","Fonts\\"..TRP2_font, 21);
	TRP2_GuideFrameTexteA:SetFont("h2","Fonts\\"..TRP2_font, 17);
	TRP2_GuideFrameTexteA:SetFont("h3","Fonts\\"..TRP2_font, 13);
	TRP2_GuideFrameTexteA:SetFont("Fonts\\"..TRP2_font,11);
	...
	TRP2_GuideFrameTexteB:SetFont("h1","Fonts\\"..TRP2_font, 21);
	TRP2_GuideFrameTexteB:SetFont("h2","Fonts\\"..TRP2_font, 17);
	TRP2_GuideFrameTexteB:SetFont("h3","Fonts\\"..TRP2_font, 13);
	TRP2_GuideFrameTexteB:SetFont("Fonts\\"..TRP2_font,11);
	...
	if UnitIsPlayer("target") then
		...
		if TRP2_SimplifyNameIfPlayer(nom) == TRP2_Joueur then
			...
			TRP2_SetTooltipForFrame(TRP2_TargetButton,TRP2_TargetButton,"BOTTOMRIGHT",10,0,
				"{v}"..TRP2_nomComplet(TRP2_Joueur,true));
...
function TRP2_RefreshTooltipForFrame(Frame)
	if Frame.tooltipTexte and Frame.GenFrame and Frame.GenFrameX and Frame.GenFrameY and Frame.GenFrameAnch then
		...
		if not Frame.texteDroite then
			...
			TRP2_MainTooltipTextRight1:SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("AideTaille",10)+4);
			TRP2_MainTooltipTextRight1:SetNonSpaceWrap(true);
		end
		TRP2_MainTooltipTextLeft1:SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("AideTaille",10)+4);
		...
		if Frame.tooltipTexteSuite then
			...
			TRP2_MainTooltipTextLeft2:SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("AideTaille",10));
...
function TRP2_MouseOverTooltip(TT_Type)
	if TRP3_API or not TRP2_GetConfigValueFor("UseTRPTooltip",true) or (UnitAffectingCombat("player") and not TRP2_GetConfigValueFor("UseTTInCombat",true)) then
		return;
	end
...
function TRP2_HookAll()
	...
	-- Version 1.030 : Compatibility with Total RP 3
	if TRP3_API then
		return;
	end
