...
function TRP2_Hooking_FrameEvent()
	function TRP2_ChatFrame_OnEvent( self, event, ... )
		...
		-- Récupération de coloredName uniquement dans un contexte d'échanges de message (post 5.4.7). Un gros bisou à Telko pour m'avoir soufflé ça
		...
		if Affiche ~= 0 and (event=="CHAT_MSG_SAY" or event=="CHAT_MSG_PARTY" or event=="CHAT_MSG_RAID" or (event=="CHAT_MSG_GUILD" and not TRP2_GetConfigValueFor("GuildNoIC",false)) or event=="CHAT_MSG_YELL"
			...
			if TRP2_GetConfigValueFor("UseTitleInChat",true) then
				if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(arg2),"Registre",{}),"Titre")) then -- On ajoute le nom si il existe
					personnage = TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(arg2),"Registre",{}),"Titre").." ";
				end
			end
			
			-- on prend le prénom si il existe
			personnage = personnage..TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(arg2),"Registre",{}),"Prenom",TRP2_RemoveRealm(arg2));
			if TRP2_GetConfigValueFor("UseNameInChat",true) then
				if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(arg2),"Registre",{}),"Nom")) then -- On ajoute le nom si il existe
					personnage = personnage.." "..TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(arg2),"Registre",{}),"Nom");
				end
			end
			...
			personnage = "[|Hplayer:"..TRP2_SimplifyNameIfPlayer(arg2).."|h"..color..personnage.."|h|r]"; -- Lien de chat
			...
			if string.find(texte,"^%[.-%]") then -- Ca cause dans un langage perso, car ya une entête
				...
				if TRP2_SimplifyNameIfPlayer(arg2) == TRP2_Joueur or event=="CHAT_MSG_WHISPER_INFORM" then -- Si c'est toi qui a causé.
					if TRP2_EmptyToNil(TRP2_DernierePhrasePerso) then
						if TRP2_GetConfigValueFor("ColorHRPMode",2) > 1 then -- Détection des HRP
							...
							elseif TRP2_GetConfigValueFor("ColorHRPMode",2) == 3 then
								...
								if getglobal("ChatFrame"..TRP2_GetConfigValueFor("HRPFrame",1)) then
									for key,value in pairs(tab) do
										TRP2_Afficher(TRP2_SimplifyNameIfPlayer(arg2).." "..TRP2_LOC_StatutHRP.."{888888} : "..value,TRP2_GetConfigValueFor("HRPFrame",1));
									end
								end
							end
						end
			   ...
			   if event~="CHAT_MSG_WHISPER_INFORM" and TRP2_SimplifyNameIfPlayer(arg2) ~= TRP2_Joueur and not string.find(TRP2_DIALBASETAB[TRP2_enRace],arg3) then
...
function TRP2_AnalyserEmote(emote,perso,myChatFrame,event,coloredName)
	...
	elseif event == "CHAT_MSG_TEXT_EMOTE" then
		...
		-- on prend le titre si il existe
			if TRP2_GetConfigValueFor("UseTitleInChat",true) then
				if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Titre")) then -- On ajoute le titre si il existe
					personnage = TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Titre").." ";
				end
			end
		-- on prend le prénom si il existe
		...
		if TRP2_GetConfigValueFor("UseNameInChat",true) then
			if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Nom")) then -- On ajoute le nom si il existe
				personnage = personnage.." "..TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Nom");
			end
		end
		if TRP2_GetConfigValueFor("UseColorInChat",true) then
			else
		...
		personnage = "|Hplayer:"..TRP2_SimplifyNameIfPlayer(perso).."|h"..color..personnage.."|h"; -- Lien de chat
		if TRP2_SimplifyNameIfPlayer(perso) == TRP2_Joueur then
	...
	elseif event == "CHAT_MSG_EMOTE" then
		...
		-- on prend le prénom si il existe
		prenom = TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Prenom",TRP2_RemoveRealm(perso));
		personnage = personnage..prenom;
		if TRP2_GetConfigValueFor("UseNameInChat",true) then
			if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Nom")) then -- On ajoute le nom si il existe
				personnage = personnage.." "..TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(perso),"Registre",{}),"Nom");
