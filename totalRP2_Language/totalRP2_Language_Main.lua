...
function TRP2_ReceiveTranslate(sender,reponseTab)
    ...
    -- on prend le titre si il existe
      if TRP2_GetConfigValueFor("UseTitleInChat",true) then
        if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(sender),"Registre",{}),"Titre")) then -- On ajoute le titre si il existe
          personnage = TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(sender),"Registre",{}),"Titre").." ";
        end
      end
    -- on prend le prénom si il existe
    prenom = TRP2_GetWithDefaut(TRP2_GetInfo(sender,"Registre",{}),"Prenom",TRP2_RemoveRealm(sender));
    personnage = personnage..prenom;
    if TRP2_GetConfigValueFor("UseNameInChat",true) then
      if TRP2_EmptyToNil(TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(sender),"Registre",{}),"Nom")) then -- On ajoute le nom si il existe
        personnage = personnage.." "..TRP2_GetWithDefaut(TRP2_GetInfo(TRP2_SimplifyNameIfPlayer(sender),"Registre",{}),"Nom");
      end
    end
	local color = "";
