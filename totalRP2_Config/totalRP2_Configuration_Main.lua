		...
		["Composants"] = {
			...
			{ -- Choix langue
				...
				["OnChange"] = function(self)
					...
					elseif value == 5 then
					TRP2_Module_Configuration[TRP2_Royaume][TRP2_Joueur][self.VarName] = 5; -- zhCN
					TRP2ConfigSliderLangueTexte:SetText("汉语");
					if self:IsVisible() then
						StaticPopupDialogs["TRP2_CONFIRM_ACTION_NS"].text = TRP2_CTS(TRP2_ENTETE.."更改将在重新加载界面后生效。\n现在重新加载?");
						TRP2_ShowStaticPopup("TRP2_CONFIRM_ACTION_NS",nil,function()
							ReloadUI();
						end);
					end
