...
function TRP2_TraiterTooltipPerso(Nom,infoTab,j,TT_Type)
	...
	if persoTab then
		...
		getglobal(TRP2_PersoTooltip:GetName().."TextLeft" .. i):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleTitre",16));
		getglobal(TRP2_PersoTooltip:GetName().."TextRight" .. i):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleTitre",16));
		i = i + 1;
		-- Titre complet
		if TRP2_GetConfigValueFor("TTPersoCompoSousTitre",true) then
			if TRP2_GetWithDefaut(registreTab,"TitreComplet") then
				...
				getglobal(TRP2_PersoTooltip:GetName().."TextLeft" .. i):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleInfo",12));
				i = i + 1;
		...
		if TRP2_GetConfigValueFor("TTPersoCompoCRLvl",true) then
			...
			getglobal(TRP2_PersoTooltip:GetName().."TextLeft" .. i):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleInfo",12));
			getglobal(TRP2_PersoTooltip:GetName().."TextRight" .. i):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleInfo",12));
			i = i + 1;
		end
		--  Royaume
		local prout,royaume = UnitName(TT_Type)
		...
		if TRP2_GetConfigValueFor("TTPersoCompoActu",true) and TRP2_GetWithDefaut(actuTab,"ActuTexte") then
			...
			getglobal(TRP2_PersoTooltip:GetName().."TextLeft" .. i):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleDescri",10));
			i = i + 1;
		end
		-- Actuellement HRP
		...
		if TRP2_GetConfigValueFor("TTPersoCompoClient",true) then
			if Nom == TRP2_Joueur then
				...
				getglobal(TRP2_PersoTooltip:GetName().."TextLeft" .. i):SetFont("Fonts\\"..TRP2_font,8);
				getglobal(TRP2_PersoTooltip:GetName().."TextRight" .. i):SetFont("Fonts\\"..TRP2_font,8);
				i = i + 1;
		...
		if TRP2_GetConfigValueFor("TTPersoCompoMonture",true) then
			...
			while(monture) do
				...
				if monture and petsTab[monture] then
					...
					getglobal(TRP2_MountTooltip:GetName().."TextLeft" .. 1):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleTitre",16));
					
					TRP2_MountTooltip:AddLine(TRP2_CTS("{w}< "..TRP2_FT(TRP2_LOC_SMOUNT,Nom).."{w} >"));
					getglobal(TRP2_MountTooltip:GetName().."TextLeft" .. 2):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleInfo",12));
					
					if TRP2_GetWithDefaut(petsTab,"Description") then
						-- DESCRIPTION
						TRP2_MountTooltip:AddLine(TRP2_CTS("{w}"..TRP2_LOC_Description.." :"));
						getglobal(TRP2_MountTooltip:GetName().."TextLeft" .. 3):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleInfo",12));
						-- DESCRIPTION CONTENU
						TRP2_MountTooltip:AddLine(TRP2_CTS("{o}\""..TRP2_GetWithDefaut(petsTab,"Description").."{o}\""),1,1,1,1);
						getglobal(TRP2_MountTooltip:GetName().."TextLeft" .. 4):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleDescri",10));
					end
					-- Auras
					if true then
						if TRP2_GetIndexedTabSize(TRP2_GetWithDefaut(petsTab,"AurasTab",{})) > 0 then
							...
							if string.gsub(auraText,"%s","") ~= "" then
								...
								getglobal(TRP2_MountTooltip:GetName().."TextLeft" .. 5):SetFont("Fonts\\"..TRP2_font,TRP2_GetConfigValueFor("TTPersoTailleInfo",12));
...
function TRP2_SetFicheGeneralConsulte(Nom)
	...
	message = "";
	local taille = TRP2_GetWithDefaut(registreTab,"Taille",3);
	if type(taille) == "number" then
		message = message.."{o}"..TRP2_LOC_TAILLE.." : {w}"..TRP2_LOC_TAILLE_TEXTE[taille].."\n";
	else
		message = message.."{o}"..TRP2_LOC_TAILLE.." : {w}"..taille.."\n";
	end
	local silouhette = TRP2_GetWithDefaut(registreTab,"Silhouette",2);
	if type(silouhette) == "number" then
		message = message.."{o}"..TRP2_LOC_SILHOUETTE.." : {w}"..TRP2_LOC_SILHOUETTE_TEXTE[silouhette].."\n";
	else
		message = message.."{o}"..TRP2_LOC_SILHOUETTE.." : {w}"..silouhette.."\n";
	end
...
function TRP2_SetFichePhysiqueSave()
	...
	tab["PhysiqueTexte"] = string.gsub(TRP2_FicheJoueurPhysiqueEditBox:GetText(),"[%#%~%µ%$%@]","");
...
function TRP2_SetFicheHistoireSave()
	...
	tab["HistoireTexte"] = string.gsub(TRP2_FicheJoueurHistoireEditBox:GetText(),"[%#%~%µ%$%@]","");
...
function TRP2_MascotteScript(nom)
	for i=1,C_PetJournal.GetNumPets() do
		...
		if owned then
			if nom == customName or nom == speciesName then
				C_PetJournal.SummonPetByGUID(petID);
				return;
			end
		end
	end
	for k=1, C_MountJournal.GetNumMounts() do

		local creatureName, creatureSpellID, icon, issummoned, isUsable, sourceType, isFavorite, isFactionSpecific, faction, hideOnChar, isCollected = C_MountJournal.GetMountInfo(k);
		if isCollected then
			if nom == creatureName then
				C_MountJournal.Summon(k);
				return;
			end
		end
	end
...
function TRP2_GetActualMinionName(default)
	...
	for i=1,totalPets do
		local petID, speciesID, owned, customName, level, favorite, active, speciesName, icon, petType, companionID, tooltip, description, isWild, canBattle, isTradeable, isUnique, obtainable = C_PetJournal.GetPetInfoByIndex(i)
		if owned then
			if active then
...
function TRP2_GetActualMinionInfo(info,default)
	for i=1,C_PetJournal.GetNumPets() do
		local petID, speciesID, owned, customName, level, favorite, active, speciesName, icon, petType, companionID, tooltip, description, isWild, canBattle, isTradeable, isUnique, obtainable = C_PetJournal.GetPetInfoByIndex(i)
		if active then
			local petTab = TRP2_GetInfo(TRP2_Joueur,"Pets",{})[customName or speciesName];
...
function TRP2_DD_PotraitIconeClick(frame,level,menuList)
    ...
    if TRP2CHATNAME then
       ...
       ReportPlayer("spam","pending","");
            ...
            HelpFrame_ShowReportPlayerNameDialog(nom);
        	else
			...
			-- Report for cheating
			info = TRP2_CreateSimpleDDButton();
			info.text = REPORT_CHEATING;
			info.func = function() 
				if ( GMQuickTicketSystemEnabled() and not GMQuickTicketSystemThrottled() ) then
            HelpFrame_ShowReportCheatingDialog(nom);
        else
            UIErrorsFrame:AddMessage(ERR_REPORT_SUBMISSION_FAILED , 1.0, 0.1, 0.1, 1.0);
            local info = ChatTypeInfo["SYSTEM"];
            if ( dropdownFrame.chatFrame ) then
