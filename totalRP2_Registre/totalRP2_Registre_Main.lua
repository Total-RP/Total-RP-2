	...
	if Nom == TRP2_Joueur and TRP2_PetsListAllCheck:GetChecked() then
		...
		for i=1,C_PetJournal.GetNumPets() do
			...
			if owned then
				...
				if not TRP2_PetsListeTab[customName or speciesName] then
					i = i + 1;
					if not search or TRP2_ApplyPattern(customName or speciesName,search) then
						j = j + 1;
						TRP2_PetsListeTab[customName or speciesName] = icon;
					end
				end
			end
		end
...
function TRP2_EstDansLeReg(nom)
	if not string.find(nom,"-") then
		nom = nom.."-"..GetRealmName():gsub("%s+", "");
	end
...
function TRP2_UpdateRegistre()
	...
	if TRP2_EstDansLeReg(nom) and not TRP2_Module_Registre[TRP2_Royaume][nom] == nil then
...
-- Modified by Ellypse in version 1.027
-- This function was previously checking for the ["Seen"] tag
-- For some reason, since the connected realms system, this tag is not behaving well.
-- We are now looking at the client, as it is only present if we've seen the character
-- so it should be a good work around, I hope !
function TRP2_EpurerRegistre()
	local count = 0;
	for p,v in pairs(TRP2_Module_Registre[TRP2_Royaume]) do
...
function TRP2_MSP_OnInformationReceived(sender)
	...
	-- Version 1.030 : Compatiblity with Total RP 3
	if TRP3_API then
		return;
	end
	...
	if msptrp.char[sender].field['VA'] ~= "" and string.sub(msptrp.char[sender].field['VA'],1,8) ~= "TotalRP2" then
		...
		for field,value in pairs(msptrp.char[sender].field) do
			...
			elseif field == "IC" then
				TRP2_Module_Registre[TRP2_Royaume][sender]["Actu"]["PlayerIcon"] = TRP2_EmptyToNil(value);
			...
			-- Added by Ellypse in version 1.030. Support for custom class and weight field.
			elseif field == "RC" then
				TRP2_Module_Registre[TRP2_Royaume][sender]["Registre"]["ClassePerso"] = TRP2_EmptyToNil(value);
			elseif field == "AH" then
			TRP2_Module_Registre[TRP2_Royaume][sender]["Registre"]["Taille"] = TRP2_EmptyToNil(value);
			elseif field == "AW" then
				TRP2_Module_Registre[TRP2_Royaume][sender]["Registre"]["Silhouette"] = TRP2_EmptyToNil(value);
			...
			elseif field == "CO" then
				TRP2_Module_Registre[TRP2_Royaume][sender]["Actu"]["ActuTexteHRP"] = TRP2_EmptyToNil(value);
			end
		end
		-- Refresh
...
function TRP2_MSP_Request(nom)
	...
	if not TRP2_EstIgnore(nom) and msptrp.char[ nom ].supported ~= false and string.sub(msptrp.char[nom].field['VA'],1,8) ~= "TotalRP2" then
		-- To do : AH
			local requested = msptrp:Request( nom, {"VA","NA","HH","NT","RA","FR","FC","AG","AE","HB","DE","HI","CU","AW","RC","IC","AH","CU", "CO"} );
...
function TRP2_MSP_InitialLoad()
	...
	if TRP3_API then
		print("|cffff0000[TRP2]|r : |cffbb2613Total RP 3|r is running. |cff009a73Total RP 2|r is now in compatibility mode (no chat, no tooltip, no MSP)");
		return;
	end
...
function TRP2_MSP_ActuUpdate(actuTab,noUpdate)
	...
	msptrp.my['IC'] = TRP2_GetWithDefaut(actuTab,"PlayerIcon");
	-- RP status
	...
	-- Currently OOC
	msptrp.my['CO'] = TRP2_CTS(TRP2_GetWithDefaut(actuTab,"ActuTexteHRP",""));
	TRP2_MSP_Update(noUpdate);
end
...
function TRP2_MSP_RegistreUpdate(registreTab,noUpdate)
	...
	-- Silouhette
	msptrp.my['AW'] = TRP2_LOC_SILHOUETTE_TEXTE[TRP2_GetWithDefaut(registreTab,"Silhouette",2)];
	-- Classe
	local localclass,englishClass = UnitClass("player");
	msptrp.my['RC'] = TRP2_GetWithDefaut(registreTab,"ClassePerso") or localclass;
	-- Home
